...
use crate::swarm::tasks::TaskStatus;
...
fn render_tabs(f: &mut Frame, app: &App, area: Rect) {
    ...
    let tabs = Tabs::new(tab_titles)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title("Forge Swarm Orchestration"),
        )
...
fn render_help(f: &mut Frame, area: Rect) {
    let help_text = vec![
        ...
        Line::from("  swarm orchestration system. Use it to view agent status, track tasks,"),
        Line::from("  monitor messages, and interact with the swarm."),
        Line::from(""),
