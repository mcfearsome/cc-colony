...
use crate::swarm::agent::AgentStatus;
use crate::swarm::messaging::Message;
use crate::swarm::tasks::{Task, TaskStatus};
use crate::swarm::SwarmConfig;

/// Represents the current state of the swarm
#[derive(Debug, Clone)]
pub struct SwarmData {
    pub agents: Vec<AgentInfo>,
    pub tasks: HashMap<TaskStatus, Vec<Task>>,
    pub messages: Vec<Message>,
    pub swarm_root: PathBuf,
}
...
impl SwarmData {
    /// Load swarm data from disk
    pub fn load(config_path: &Path) -> Result<Self, String> {
        let swarm_root = PathBuf::from(".forge-swarm");

        if !swarm_root.exists() {
            return Err("Swarm not initialized. Run 'forge swarm start' first.".to_string());
        }

        // Load configuration
        let config = SwarmConfig::load(config_path)
            .map_err(|e| format!("Failed to load swarm config: {}", e))?;

        // Load agents
        let agents = Self::load_agents(&config, &swarm_root)?;

        // Load tasks
        let tasks = Self::load_tasks(&swarm_root)?;

        // Load messages
        let messages = Self::load_messages(&swarm_root)?;
    ...
    fn load_agents(config: &SwarmConfig, swarm_root: &Path) -> Result<Vec<AgentInfo>, String> {
        let mut agents = Vec::new();

        // Load state file if it exists
        let state_path = swarm_root.join("state.json");
        ...
        let task_assignments = Self::load_task_assignments(swarm_root)?;
    ...
    fn load_task_assignments(swarm_root: &Path) -> Result<HashMap<String, String>, String> {
        let mut assignments = HashMap::new();
        let in_progress_dir = swarm_root.join("tasks/in_progress");
    ...
    fn load_tasks(swarm_root: &Path) -> Result<HashMap<TaskStatus, Vec<Task>>, String> {
        let mut tasks_by_status = HashMap::new();
        let tasks_dir = swarm_root.join("tasks");
    ...
    fn load_messages(swarm_root: &Path) -> Result<Vec<Message>, String> {
