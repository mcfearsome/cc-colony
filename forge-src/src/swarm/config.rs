...
/// Configuration for a swarm of Claude Code agents
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SwarmConfig {
    /// Optional name for this swarm (defaults to directory name)
    #[serde(default)]
...
impl SwarmConfig {
    ...
    pub fn load(path: &Path) -> ForgeResult<Self> {
        ...
        let config: SwarmConfig = serde_yaml::from_str(&contents)?;
        Ok(config)
    }

    /// Save swarm configuration to a YAML file
    ...
    /// Create a default swarm configuration with example agents
    pub fn default() -> Self {
        SwarmConfig {
