...
pub async fn run() -> ForgeResult<()> {
    ...
    utils::header("Destroying Forge Swarm");
    ...
    if swarm_root.exists() {
        let spinner = utils::spinner("Removing swarm directory...");
        std::fs::remove_dir_all(swarm_root)?;
        spinner.finish_and_clear();
        utils::success("Removed swarm directory");
    }

    utils::success("Swarm destroyed successfully");
    utils::info("\nThe swarm.yml file has been preserved.");
    utils::info("Run 'forge swarm start' to recreate the swarm.");
