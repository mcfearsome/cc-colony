...
pub fn create_worktree(agent_id: &str, base_path: &Path) -> ForgeResult<PathBuf> {
...
pub fn remove_worktree(worktree_path: &Path) -> ForgeResult<()> {
    if !worktree_path.exists() {
        return Ok(());
    }
...
/// List all worktrees
pub fn list_worktrees() -> ForgeResult<Vec<PathBuf>> {
    let output = Command::new("git")
        .arg("worktree")
        .arg("list")
        .arg("--porcelain")
