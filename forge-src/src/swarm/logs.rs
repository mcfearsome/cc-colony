...
pub async fn run(agent_id: Option<String>) -> ForgeResult<()> {
...
fn show_agent_logs(controller: &SwarmController, agent_id: &str) -> ForgeResult<()> {
...
fn list_agent_logs(controller: &SwarmController) -> ForgeResult<()> {
    ...
    println!();
    utils::info("Use 'forge swarm logs <agent-id>' to view specific logs");
