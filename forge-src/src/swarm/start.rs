...
use crate::swarm::{messaging, tmux, AgentStatus, SwarmConfig, SwarmController};
use crate::utils;

/// Start all agents in the swarm
pub async fn run(no_attach: bool) -> ForgeResult<()> {
    ...
    utils::header("Starting Forge Swarm");
    ...
    for (index, agent_id) in agent_ids.iter().enumerate() {
        ...
        let worktree_path_str = agent.worktree_path.to_str().ok_or_else(|| {
            crate::error::ForgeError::Swarm(format!(
    ...
    let forge_binary = std::env::current_exe()
        .map_err(|e| crate::error::ForgeError::Swarm(format!("Failed to get forge binary path: {}", e)))?;
    let forge_path = forge_binary.to_str().ok_or_else(|| {
        crate::error::ForgeError::Swarm("Forge binary path contains non-UTF-8 characters".to_string())
    })?;

    // Get current directory for the TUI to run in
    let current_dir = std::env::current_dir()
        .map_err(|e| crate::error::ForgeError::Swarm(format!("Failed to get current directory: {}", e)))?;
    let current_dir_str = current_dir.to_str().ok_or_else(|| {
        crate::error::ForgeError::Swarm("Current directory path contains non-UTF-8 characters".to_string())
    })?;
    ...
    utils::header("Swarm Started Successfully!");
    ...
    if no_attach {
        println!("\nNext steps:");
        println!("  • Run 'forge swarm attach' to view agents and TUI in tmux");
        println!("  • The orchestration TUI is already running in a dedicated pane");
        println!("  • Use 'forge swarm status' to check agent status from CLI");
        println!("  • Use 'forge swarm logs <agent-id>' to view specific logs");
...
async fn create_startup_prompt(agent: &crate::swarm::Agent) -> ForgeResult<()> {
    let prompt_path = agent.project_path.join("startup_prompt.txt");
    let prompt = format!(
        r#"# Welcome to Forge Swarm

You are **{}** working as part of a multi-agent swarm.
...
./swarm_message.sh send <agent-id> "Your message here"

# Broadcast to all agents
./swarm_message.sh send all "Important announcement"
```
...
When starting work:
```bash
./swarm_message.sh send all "Starting work on [task description]"
```

When you need help:
```bash
./swarm_message.sh send <agent-id> "Need assistance with X"
```

When you complete something:
```bash
./swarm_message.sh send all "Completed [task description] - ready for integration"
```

## Coordination

Read the full communication guide at:
`.forge-swarm/SWARM_COMMUNICATION.md`
...
/// Set up messaging infrastructure for the swarm
fn setup_messaging_infrastructure(controller: &SwarmController) -> ForgeResult<()> {
    let swarm_root = controller.swarm_root();
    ...
    messaging::create_messaging_readme(swarm_root)?;

    // Create helper scripts for each agent
    for agent in controller.agents().values() {
        messaging::create_message_helper_script(swarm_root, agent.id())?;
    }
