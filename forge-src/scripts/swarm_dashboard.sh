#!/bin/bash
# Forge Swarm Live Dashboard
# Displays real-time swarm status, task progress, and activity

SWARM_ROOT="${1:-.forge-swarm}"
...
get_recent_messages() {
    local messages_dir="$SWARM_ROOT/messages"
...
show_dashboard() {
    ...
    # Header
    echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}FORGE SWARM DASHBOARD${NC}                                      ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
    echo
    ...
    # Task Statistics
    if [ -d "$SWARM_ROOT/tasks" ]; then
        echo -e "${BOLD}${YELLOW}📋 TASK STATUS${NC}"
        echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

        local pending=$(count_files "$SWARM_ROOT/tasks/pending")
        local claimed=$(count_files "$SWARM_ROOT/tasks/claimed")
        local in_progress=$(count_files "$SWARM_ROOT/tasks/in_progress")
        local blocked=$(count_files "$SWARM_ROOT/tasks/blocked")
        local completed=$(count_files "$SWARM_ROOT/tasks/completed")
    ...
    # Agent Status
    if [ -d "$SWARM_ROOT/worktrees" ]; then
        echo -e "${BOLD}${GREEN}👥 ACTIVE AGENTS${NC}"
        echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

        # Pre-build agent->task mapping (O(M) instead of O(N*M))
        declare -A agent_tasks
        if [ -d "$SWARM_ROOT/tasks/in_progress" ]; then
            for task_file in "$SWARM_ROOT/tasks/in_progress"/*.json; do
        ...
        for agent_dir in "$SWARM_ROOT/worktrees"/*; do
            if [ -d "$agent_dir" ]; then
    ...
    if [ -d "$SWARM_ROOT/messages" ]; then
        echo -e "${BOLD}${CYAN}📬 RECENT ACTIVITY${NC}"
        echo -e "${BOLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

        local msg_count=$(find "$SWARM_ROOT/messages" -name "*.json" -type f -mmin -5 2>/dev/null | wc -l | tr -d ' ')

        if [ "$msg_count" -gt 0 ]; then
            find "$SWARM_ROOT/messages" -name "*.json" -type f -mmin -5 2>/dev/null | \
...
# Main loop
echo "Starting Forge Swarm Dashboard..."
echo "Monitoring: $SWARM_ROOT"
